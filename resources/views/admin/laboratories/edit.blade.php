...
@section('content')
<div class="mb-4">
    <a href="{{ route('admin.laboratories.index') }}" class="btn btn-sm btn-secondary mb-3">
                ...
                <form action="{{ route('admin.laboratories.update', $laboratory) }}" method="POST">
                        ...
                        <select class="form-select @error('department_id') is-invalid @enderror"
                            ...
                            @foreach($departments as $dept)
                                <option value="{{ $dept->department_id }}"
                                    {{ old('department_id', $laboratory->department_id) == $dept->department_id ? 'selected' : '' }}>
                                    {{ $dept->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control @error('location') is-invalid @enderror"
                               id="location" name="location" value="{{ old('location', $laboratory->location) }}" required>
